...
/**
 * Finds the company that owns a specific WhatsApp phone_number_id.
 * Used by the inbound webhook to route messages to the correct tenant.
 */
export async function getCompanyByPhoneNumberId(phoneNumberId: string) {
  ...
  return (data?.company as unknown as Record<string, unknown>) || null;
}

/**
 * Returns the first active AI agent for a company.
 * Used by automations and WhatsApp auto-reply.
 */
